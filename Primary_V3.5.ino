/*
 * Arduino NANO ESP32-S3 Fish Tank Primary Control System v3.5
...
void updateDisplay() {
    ...
    bool shouldBeOn = shouldOLEDBeOn();
    
    // Turn off display if it should be off
    if (!shouldBeOn && oledIsOn) {
        display.clearDisplay();
        display.display();
        display.ssd1306_command(SSD1306_DISPLAYOFF);
        oledIsOn = false;
        Serial.println("OLED turned OFF (scheduled time)");
        return;
    }
    
    // Turn on display if it should be on but currently off
    if (shouldBeOn && !oledIsOn) {
        display.ssd1306_command(SSD1306_DISPLAYON);
        oledIsOn = true;
        Serial.println("OLED turned ON (scheduled time)");
    }
    
    // Only update content if display should be on
    if (shouldBeOn) {
        display.clearDisplay();
        ...
        // Display mDNS address
        display.setCursor(0, 55);
        display.println(F("http://fishbreedtanks.local"));
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("=== Fish Tank Control System v3.4 Starting ===");
    ...
    queueSlackMessage("Fish Tank Control System v3.4 started");
