/*
 * Arduino NANO ESP32-S3 Fish Tank Primary Control System v4.1
 * Manages up to 25 fish breeding tanks via ESP-NOW and web interface
 *
 * NEW IN v4.1:
 * - FIXED: Timer state now preserved when Secondary is in deep sleep
 * - FIXED: Tanks with active timers no longer marked as "Disconnected"
 * - FIXED: Web interface shows accurate timer status throughout sleep period
 * - FIXED: Connection timeout skips tanks with active timers (expected to be asleep)
 * - FIXED: Timer tracking starts before sending command to prevent race condition
 * - FIXED: Added grace period after timer expiry to wait for Secondary confirmation
 *
 * NEW IN v4.0:
 * - FIXED: Removed duplicate esp_now_init() call
 * - FIXED: Removed unused fish bitmap (saved 264 bytes PROGMEM)
 * - FIXED: Sleep-after-opening now sends configuration command to Secondary
 * - FIXED: Changed timing variables to uint32_t to prevent overflow (max 5 days)
 * - FIXED: Moved tank ID validation before array access in ESP-NOW callback
 * 
 * NEW IN v3.9:
 * - Changed timer display to show actual wake/open time instead of countdown
 * - Removed "Cancel all timed openings" button (incompatible with sleep-enabled timers)
 * - Removed individual "Cancel delay" buttons (tanks sleep immediately)
 * 
 * Required Libraries:
...
uint8_t secondaryMACAddresses[25][6] = {
  {0x30, 0xED, 0xA0, 0xAC, 0xAB, 0xE8}, // secondary1
  {0x94, 0xA9, 0x90, 0x17, 0x84, 0x70}, // secondary2
  {0x30, 0xED, 0xA0, 0xAC, 0xAC, 0xC0}, // secondary3
  {0x30, 0xED, 0xA0, 0xAC, 0xAE, 0x78}, // secondary4
  {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0x05}, // secondary5
  {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0x06}, // secondary6
  {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0x07}, // secondary7
  {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0x08}, // secondary8
  {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0x09}, // secondary9
...
uint32_t delayStartTime[MAX_TANKS] = {0};      // FIXED v4.0: Changed to uint32_t (5 days = 432M ms fits)
int delayDuration[MAX_TANKS] = {0};
uint32_t lastHeartbeat[MAX_TANKS] = {0};       // FIXED v4.0: Changed to uint32_t
bool tankInSleep[MAX_TANKS] = {false};         // REF10
uint32_t sleepStartTime[MAX_TANKS] = {0};      // FIXED v4.0: Changed to uint32_t
int sleepDuration[MAX_TANKS] = {0};
bool sleepAfterOpening[MAX_TANKS] = {false};   // Track if tank should sleep after opening
...
// display off times
const int OLED_OFF_HOUR = 19;  // 7pm (24-hour format)
const int OLED_ON_HOUR = 6;    // 6am (24-hour format)
bool oledIsOn = true;          // Track OLED state

// Slack non blocking
#include <queue>
std::queue<String> slackQueue;
uint32_t lastSlackSend = 0;    // FIXED v4.0: Changed to uint32_t
 ...
 * Input: None (uses global ssid and password)
 * Output: None
 */
void initWiFi() {
 ...
 * Input: None (uses global NTP settings)
 * Output: None
 */
void initTime() {
...
/**
 * Add all secondary tank devices as ESP-NOW peers
 * Input: None (uses global secondaryMACAddresses array)
 * Output: None
 */
void addESPNowPeers() {
 ...
 * Input: None
 * Output: String - formatted time string in PST
 */
...
/**
 * Calculate and format tank wake-up time from sleep
 * Input: arrayIndex - int tank array index (0-24)
 * Output: String - formatted wake time or empty string
 */
String getWakeUpTime(int arrayIndex) {
    ...
    time_t now;
    time(&now);
    uint32_t elapsed = millis() - sleepStartTime[arrayIndex];  // FIXED v4.0: uint32_t
    uint32_t remaining = ((uint32_t)sleepDuration[arrayIndex] * 3600000) - elapsed;  // FIXED v4.0: explicit cast
...
/**
 * Calculate and format timed opening execution time
 * Input: arrayIndex - int tank array index (0-24)
 * Output: String - formatted open time or empty string
 */
String getTimedOpenTime(int arrayIndex) {
    ...
    time_t now;
    time(&now);
    uint32_t elapsed = millis() - delayStartTime[arrayIndex];  // FIXED v4.0: uint32_t
    uint32_t remaining = ((uint32_t)delayDuration[arrayIndex] * 3600000) - elapsed;  // FIXED v4.0: explicit cast
...
/**
 * Callback when ESP-NOW message send completes
 * Input: mac_addr - recipient MAC, status - send status
 * Output: None
 */
...
/**
 * Callback when ESP-NOW message is received from Secondary
 * Input: mac - sender MAC, incomingData - message bytes, len - message length
 * Output: None
 */
void onESPNowDataReceived(const uint8_t *mac, const uint8_t *incomingData, int len) {
    ...
    // FIXED v4.0: Validate tank ID BEFORE using it for array access
    ...
    int arrayIndex = tankId - 1;  // Now safe to use
    ...
    switch (message.messageType) {
        case 1: // Status update / Timed open notification
        ...
        case 5: // REF9: Door status
            {
                ...
                if (tankStatus[arrayIndex] != prev) {
                    if (tankStatus[arrayIndex]) {
                        queueSlackMessage("Tank" + String(tankId) + " opened the slot/door");
                        
                        // Sleep-after-opening is now handled on Secondary side
                        // The setting has already been sent via set_sleep_after_open command
                    } else {
                        queueSlackMessage("Tank" + String(tankId) + " closed the slot/door");
                    }
                }
            }
            break;
...
/**
 * Send command to specific tank via ESP-NOW
 * Input: tankId - int tank number (1-25), command - String command name, value - int optional value
 * Output: None
 */
 ...
 * Input: command - String command name
 * Output: None
 */
...
/**
 * Test close specific tank
 * Input: tankId - int tank number (1-25)
 * Output: None
 */
...
/**
 * Start delay timer for timed opening
 * Input: tankId - int tank number (1-25), hours - int hours to delay (1-120)
 * Output: None
 */
void startDelayTimer(int tankId, int hours) {
    ...
    // FIXED v4.1: Set tracking flags BEFORE sending command
    // This ensures Primary knows about timer even if Secondary sleeps immediately
...
/**
 * Cancel all active timed openings
 * Input: None
 * Output: None
 */
...
/**
 * Cancel delay timer for specific tank
 * Input: tankId - int tank number (1-25)
 * Output: None
 */
...
/**
 * Send sleep command to tank
 * Input: tankId - int tank number (1-25), hours - int hours to sleep (1-120)
 * Output: None
 */
...
/**
 * Clear all error messages from all tanks
 * Input: None
 * Output: None
 */
...
/**
 * Check and update delay timer states
 * Input: None
 * Output: None
 */
void checkDelayTimers() {
    for (int i = 0; i < MAX_TANKS; i++) {
        if (delayRunning[i]) {
            ...
            // FIXED v4.1: Add grace period after timer expires
            // Wait for Secondary to wake and confirm opening before clearing timer
            if (elapsed >= duration) {
                uint32_t gracePeriod = 300000; // 5 minutes grace period
...
/**
 * Check and update sleep timer states
 * Input: None
 * Output: None
 */
...
/**
 * Check connection status and timeout disconnected tanks
 * Input: None
 * Output: None
 */
void checkConnectionStatus() {
    ...
    for (int i = 0; i < MAX_TANKS; i++) {
        // FIXED v4.1: Skip tanks with active timers or in sleep mode
        // They are EXPECTED to be disconnected during timed opening/sleep
        if (delayRunning[i] || tankInSleep[i]) {
            continue;
        }
        
        // Only mark as disconnected if no timer, not sleeping, and timeout exceeded
...
/**
 * Queue Slack message for non-blocking send
 * Input: message - String message to send
 * Output: None
 */
...
/**
 * Get tank display information for status indicator
 * Input: arrayIndex - int tank array index (0-24)
 * Output: String - formatted as "css-class|status-text"
 */
String getTankDisplayInfo(int arrayIndex) {
    // FIXED v4.1: Check timer FIRST before checking connection status
    // Tanks are expected to be disconnected during timed opening (sleeping)
    if (delayRunning[arrayIndex]) {
    ...
    // Only show disconnected if NOT in timer or sleep mode
 ...
 * Input: None
 * Output: String - complete HTML page
 */
String generateHomePage() {
    ...
    for (int i = 0; i < MAX_TANKS; i++) {
        ...
        // FIXED v4.1: Show timer info even if tank appears disconnected
        // (it's sleeping during timed opening)
        if (delayRunning[i]) {
...
/**
 * Generate HTML for tank settings page
 * Input: None
 * Output: String - complete HTML page
 */
String generateTankSettingsPage() {
    ...
    for (int i = 1; i <= MAX_TANKS; i++) {
        ...
        // FIXED v4.1: Allow clicking on timer/sleeping tanks
...
/**
 * Generate HTML for individual tank control page
 * Input: tankId - int tank number (1-25)
 * Output: String - complete HTML page
 */
String generateTankPage(int tankId) {
    ...
    // FIXED v4.1: Allow access to tank page if timer is running or sleeping
    // (not just when connected)
    ...
    html += F(".checkbox-container { margin: 15px 0; padding: 12px; background: rgba(255,255,255,0.5); border-radius: 8px; }");
    html += F(".checkbox-container label { display: flex; align-items: center; cursor: pointer; font-weight: 500; }");
    html += F(".checkbox-container input[type='checkbox'] { width: 20px; height: 20px; margin-right: 10px; cursor: pointer; }");
    html += F(".checkbox-description { font-size: 0.9em; color: #555; margin-top: 5px; margin-left: 30px; }");
    html += F("</style>");
    html += F("</head><body>");
    html += F("<div class='container'>");
    ...
    // FIXED v4.1: Show accurate connection status considering timer and sleep states
    ...
    html += F("<label><input type='checkbox' id='sleepAfterOpen");
    html += String(tankId);
    html += F("' ");
    if (sleepAfterOpening[arrayIndex]) {
        html += F("checked");
    }
    html += F(" onchange=\"toggleSleepAfterOpen(");
    html += String(tankId);
    html += F(")\"> Sleep after opening</label>");
    html += F("<div class='checkbox-description'>Tank will enter low power mode after opening slot/door</div>");
    html += F("</div>");
    ...
    html += F("function toggleSleepAfterOpen(tankId) { var checked = document.getElementById('sleepAfterOpen' + tankId).checked; fetch('/command', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'command=toggle_sleep_after_open&tank=' + tankId + '&value=' + (checked ? '1' : '0') }).then(response => { if(response.ok) { console.log('Sleep after opening ' + (checked ? 'enabled' : 'disabled')); } }).catch(() => alert('Error toggling setting')); }");
    html += F("setInterval(function(){ location.reload(); }, 60000);");
    html += F("</script>");
    html += F("</div></body></html>");
    ...
    server.on("/command", HTTP_POST, [](AsyncWebServerRequest *request) {
        try {
            if (request->hasParam("command", true)) {
                ...
                } else if (command == "toggle_sleep_after_open") {
                    int value = request->hasParam("value", true) ? request->getParam("value", true)->value().toInt() : 0;
                    if (tankId >= 1 && tankId <= MAX_TANKS) {
                        int arrayIndex = tankId - 1;
                        sleepAfterOpening[arrayIndex] = (value == 1);
                        Serial.println("Tank " + String(tankId) + " sleep after opening: " + String(sleepAfterOpening[arrayIndex] ? "ENABLED" : "DISABLED"));
                        
                        // Send configuration command to Secondary
                        sendCommandToTank(tankId, "set_sleep_after_open", value);
                    }
                }
...
/**
 * Determine if OLED should be on based on time of day
 * Input: None
 * Output: bool - true if OLED should be on
 */
bool shouldOLEDBeOn() {
    struct tm timeinfo;
    if (!getLocalTime(&timeinfo)) {
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("=== Fish Tank Control System v4.1 Starting ===");
    ...
    queueSlackMessage("Fish Tank Control System v4.1 started");
