/*
 * ESP32-S3 Fish Tank Secondary Control System v4.8
 * Individual tank controller that communicates with Primary via ESP-NOW
 *
 * CHANGES in V4.8:
 * - fix reconnect logic: split lastHeartbeat into separate lastHeartbeat and
 *   lastReconnectAttempt variables so the 5-minute reconnect check can never
 *   be starved by the 30-second heartbeat resetting the same variable
 * - fix uninitialized ESPNowMessage struct in EnterSleepCycle() (zero-init)
 *
 * CHANGES in V4.7:
 * - fix slack messaging, should now always slack on sleep with battery level
 *
 * CHANGES in V4.6:
 ...
 *
 * CHANGES in V4.5:
 * - fix in setup to make the sleep after open more robust, it was brittle in V4.4 and sometimes failed
 *
 * CHANGES in V4.4:
...
uint32_t lastReconnectAttempt = 0;  // FIX V4.8: separate timer for reconnect logic
...
void OpenDoor() {
    ...
    // Let mechanics + sensor settle a moment (optional but helpful)
    delay(200);

    // ---------------- FIX: Debounced OPEN confirmation ----------------
    int openCount = 0;
    const int samples = 9;
    ...
    bool doorConfirmedOpen = (openCount >= (samples / 2 + 1)); // majority vote
    // ------------------------------------------------------------------
    ...
    if (doorConfirmedOpen) {
        ...
        // NEW: still sleep after timed open, even on failure
        if (g_isTimedOpenExecution) {
...
void EnterSleepCycle() {
    ...
    // Notify primary that tank is entering sleep cycle, with battery level embedded
    // This fires regardless of whether the opto confirmed the door open
    {
        ESPNowMessage sleepNotify = {};  // FIX V4.8: zero-initialize all fields
        sleepNotify.tankId  = TANK_ID_NUMBER;
        sleepNotify.messageType = 1;
        sleepNotify.value   = batteryPercentage;   // fresh battery reading embedded directly
...
void setup() {
    ...
    Serial.println("=== Fish Tank Secondary v4.8 Starting ===");
    ...
    if (rtc_timedOpenActive && rtcValid) {
        ...
        // Enter sleep cycle immediately if OpenDoor flagged it, 
        // rather than deferring to loop() where it could be lost
        if (pendingSleepCycle) {
            pendingSleepCycle = false;
            Serial.println("DEBUG: Executing sleep cycle entry immediately after timed open");
            delay(500);
            EnterSleepCycle();
            // EnterSleepCycle never returns (deep sleep)
        }
    ...
    Serial.println("=== Secondary v4.8 Ready ===");
}
...
void loop() {
    ...
    // FIX V4.8: use lastReconnectAttempt instead of lastHeartbeat so the
    // 30-second heartbeat can no longer starve the 5-minute reconnect check
    if (millis() - lastReconnectAttempt > 300000) {
        Serial.println("WARNING: No primary response for 5 minutes - re-scanning for channel");
        __espNowReady = false;  // FIX V4.8: force full re-init on reconnect
        initESPNow();
