 ...
 *
 * CHANGES in V5.1.1:
 * - slight change to setup to read the ADC for the battery voltage before ESP-NOW which interferes with it
 * 
 * 
...
// Duty cycle (value/255) per tank, auto-selected from TANK_ID_NUMBER (NEW in V5.2)
int getMotorDutyForTank(int tankId) {
    switch (tankId) {
        case 1: return 150;
        case 2: return 150;
        case 3: return 150;
...
const int MOTOR_DUTY = getMotorDutyForTank(TANK_ID_NUMBER);
...
void setup() {
    ...
    initGPIO();
    BattCalcEnergy(); // Read battery before any WiFi/ESP-NOW activity
